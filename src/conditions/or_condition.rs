...
impl OrCondition {
    ...
    pub fn from_scheme(scheme: OrConditionScheme) -> Self {
        OrCondition {
            conditions: scheme
                .conditions
                .into_iter()
...
impl Condition for OrCondition {
    ...
    fn create(&self) -> Result<super::ConditionScheme, AutoPilotError> {
        ...
        loop {
            println!("{}", "[OR CONDITION]".yellow());
            if !Confirm::with_theme(&ColorfulTheme::default())
                .with_prompt("Do you want to add a condition to 'OR'? ")
                .interact_opt()
                .map_err(|err| {
        ...
        //     .collect();
        Ok(ConditionScheme::Or(OrConditionScheme {
