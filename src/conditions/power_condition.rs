...
impl PowerCondition {
    ...
    pub fn from_scheme(scheme: PowerConditionScheme) -> Self {
        Self {
            check_charging: scheme.check_charging.unwrap_or(true),
    ...
    fn check(&self) -> bool {
        #[cfg(target_os = "linux")]
        {
            if self.check_charging {
                ...
                let paths = ["/sys/class/power_supply/AC/online", "/sys/class/power_supply/ACAD/online"];
                for path in paths {
            ...
            if let Ok(output) = cmd("pmset", vec!["-g", "batt"]).read() {
                if self.check_charging {
                    ...
                    if let Some(pct_pos) = output.find('%') {
                        let start = output[..pct_pos].rfind(|c: char| !c.is_numeric()).unwrap_or(0);
        ...
        #[cfg(target_os = "windows")]
        {
            if self.check_charging {
                // 2 = AC, 1 = Battery
                if let Ok(output) = cmd("powershell", vec!["-Command", "(Get-CimInstance -ClassName Win32_Battery).BatteryStatus"]).read() {
                    return output.trim() == "2";
                }
                false
            } else if let Some(threshold) = self.threshold {
                if let Ok(output) = cmd("powershell", vec!["-Command", "(Get-CimInstance -ClassName Win32_Battery).EstimatedChargeRemaining"]).read() {
